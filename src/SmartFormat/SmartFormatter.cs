...
namespace SmartFormat
{
    ...
    public class SmartFormatter
    {
        ...
        public object EvaluateSingleSelector(Format formatParsed, params object?[] args)
        {
            using var zsOutput = new ZStringOutput(ZStringBuilderUtilities.CalcCapacity(formatParsed));
            var formatDetails = FormatDetailsPool.Instance.Get().Initialize(this, formatParsed, args, null, zsOutput);
            var current = ((IList<object?>)args).Count > 0 ? args[0] : args; // The first item is the default.
            var formattingInfo = FormattingInfoPool.Instance.Get().Initialize(formatDetails, formatParsed, current);
            return EvaluateSingleSelector(formattingInfo);
        }
        ...
        public object EvaluateSingleSelector(FormattingInfo formattingInfo)
        {
            ...
            var item = formattingInfo.Format.Items.Single();
            ...
            {
                EvaluateSelectors(childFormattingInfo);
                return childFormattingInfo.CurrentValue;
            }
            catch (Exception ex)
            {
                // An error occurred while evaluation selectors
